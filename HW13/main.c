...
{
	info_node *info = NULL;												//Двусвязный циклический список. Избыточен? Может быть. Зато удобный.
	int month=0, help=0, print_info=0, sort=0;
	char *f_input=NULL, *f_output=NULL, *t_string=NULL;
	for (int i=1; i < argc; i++){
		if (argv[i][0] == '-'){											//Использован кастомный разборщик параметров, т.к. getopt не обеспечил нужный функционал,
			switch (argv[i][1]){										//а именно, ограничения в размещении необязательных значений параметров (только слитное расположение,
				case 'h':												//а иначе читает следующий параметр как значение текущего).
					help = 1; break;
				case 'f':
					if (argv[i][2] != '\0')
						f_input = &argv[i][2];
